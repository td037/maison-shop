...
export default function UserCreateForm({ form, onChange }: UserCreateFormProps) {
  return (
    <div className="space-y-6">
      <div className="rounded-[24px] border border-outline-variant/20 bg-surface-low p-5">
        <SectionTitle title="Thông tin tài khoản" description="Tạo user mới với vai trò customer hoặc admin." />
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Mật khẩu">
            <input type="password" value={String(form.password || '')} onChange={(e) => onChange('password', e.target.value)} className="input-field w-full" placeholder="••••••••" />
          </Field>
